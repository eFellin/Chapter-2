...
surveys$RegionOFF #### outdoor work location (region) - duplicate lines when more than one region
...
surveys[which(surveys$RegionOFF=="Monteregie"),]$RegionOFF <- "Montérégie"

table(surveys$RegionOFF) ### 8 regions, so drop the ones that we do not need in geo_aggregated
...
risk_impact=aggregate(Risk.Score ~ RegionOFF, data=surveys, mean, na.rm=TRUE)   #### average impact risk by fsa
risk_likely=aggregate(outlikelihood ~ RegionOFF, data=surveys, mean, na.rm=TRUE)   #### average likelihood risk by fsa
risk_per=aggregate(OUTRS ~ RegionOFF, data=surveys, mean, na.rm=TRUE)   #### average likelihood risk by fsa
...
map_data <- left_join(map_data, risk_per, by = "RegionOFF")
names(map_data) <- c("RegionOFF", "geometry", "risk_impact", "risk_likely", "risk_per")
map_data
...
centroid_coords_df <- data.frame(cbind(centroid_coords$RegionOFF, st_coordinates(centroid_coords)))
names(centroid_coords_df) <- c("RegionOFF", "Centroid_X", "Centroid_Y")
centroid_coords_df

surveys <- left_join(surveys, centroid_coords_df, by = "RegionOFF")   ### add centroid coordinates to survey data
surveys  ### 114 observations
table(surveys$RegionOFF)
length(which(is.na(surveys$RegionOFF))) ### 20 NAs
